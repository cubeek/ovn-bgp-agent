...
class PortBindingChassisCreatedEvent(base_watcher.PortBindingChassisEvent):
    ...
    def match_fn(self, event, row, old):
        try:
            ...
            if not utils.has_ip_address_defined(row.mac[0]):
                return False
...
class SubnetRouterUpdateEvent(base_watcher.PortBindingChassisEvent):
    ...
    def match_fn(self, event, row, old):
        ...
        try:
            # single and dual-stack format
            if (not utils.has_ip_address_defined(row.mac[0]) and
                    not utils.has_ip_address_defined(old.mac[0])):
                return False
            return (
                not row.chassis and
                row.logical_port.startswith("lrp-") and
        ...
        try:
            ...
            if row.mac == ['unknown']:
                ...
                if not n_cidrs:
                    return False
            # single and dual-stack format
            elif not utils.has_ip_address_defined(row.mac[0]):
                return False
