...
class NATMACAddedEvent(base_watcher.DnatSnatBaseEvent):
    ...
    def match_fn(self, event, row, old):
        try:
            ...
            LOG.error("NAT entry %s has no logical port set.", row._uuid)
            return False
        ...
        if not row.external_ip:
            LOG.error("NAT entry %s does not have external_ip set", row._uuid)
            return False
...
class ExposeFIPOnCRLRP(base_watcher.FipOnCRLRPBaseEvent):
    ...
    def run(self, event, row, old):
        ...
        with _SYNC_STATE_LOCK.read_lock():
            self.agent.expose_fip(row.external_ip, mac, ls_name, lsp)
